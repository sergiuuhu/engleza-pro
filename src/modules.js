...
const Modules = (p) => {
  const list = Object.keys(modules[p.voice]).map((item) => ({
    title: p.say(decodeURIComponent(item)),
  ...
  return (
    <div style={styles.card}>
          ...
          >
            {p.voices.map((voice, index) => (
              <option key={index} value={voice}>
                {p.say(voice)} {voice.includes("British") ? "🇬🇧" : "🇺🇸"}
              </option>
            ))}
          </select>
        </div>
      ...
      {/* <a href="https://englishonthego.engleza.pro" style={styles.otg}>
       ...
       <div style={styles.otgDescription}>
         {p.say("Premium 1-hour+ playlists for business.")}
       </div>
     </a> */}
      ...
      <div style={styles.footer}>
        ...
        <p>
          {p.say("© 3M DIGITAL LTD 2024.")}
          <br />
          <br />
          {p.say("Registered in England and Wales (No. 11681453).")}
          <br />
          {p.say(
